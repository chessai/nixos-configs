...
in {
  options = {
    services.minecraft-server = {
      ...
      ops = mkOption {
        type = with types; listOf (attrsOf (oneOf [ minecraftUUID str int ]));
        default = [];
        example = literalExample ''
          [
            {
            ...
            }
            {
              ...
              name = "nixos_minecraft_admin";
              level = 3;
            }
          ]
        '';
        description = ''
          Minecraft ops, i.e. users with elevated permissions.
          See https://minecraft.gamepedia.com/Server for some details.

          Only has an effect when<option>services.minecraft-server.declarative</option>
          is enabled.
        '';
      };
